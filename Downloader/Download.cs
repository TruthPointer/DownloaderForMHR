...
internal class Download : IDownload
{
    ...
    public Download(int taskId, string url, string path, string filename, DownloadConfiguration configuration)
    {
        TaskId = taskId;

        _downloadService = new DownloadService(TaskId, configuration);
        Url = url;
        Folder = path;
        Filename = filename;
