...
internal class ChunkDownloader
{
    ...
    public ChunkDownloader(int taskId, Chunk chunk, DownloadConfiguration config, ConcurrentStream storage, ILogger logger = null)
    {
        TaskId = taskId;
        Chunk = chunk;
        ...
        try
        {
            ...
            while (readSize > 0 && Chunk.CanWrite)
            {
                ...
                if (readSize > 0)
                {
                    ...
                    OnDownloadProgressChanged(new DownloadProgressChangedEventArgs(TaskId, Chunk.Id) {
