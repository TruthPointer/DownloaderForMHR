...
namespace Downloader
{
    public class DownloadService : IDownloadService, IDisposable
    {
        ...
        public event EventHandler</*AsyncCompletedEventArgs*/DownloadAsyncCompletedEventArgs> DownloadFileCompleted;
        ...
        public DownloadService(int taskId, DownloadConfiguration options) : this(taskId)
        {
            if (options != null)
            {
                Options = options;
        ...
        public DownloadService(int taskId)
        {
            TaskId = taskId;
        ...
        public void ResetDownloadConfiguration(DownloadConfiguration options)
        {
            if (options != null)
            {
                Options = options;
        ...
        private async Task<Stream> StartDownload()
        {
            try
            {
                ...
                OnDownloadStarted(new DownloadStartedEventArgs(TaskId, Package.FileName, Package.TotalFileSize));
            ...
            {
                Status = DownloadStatus.Stopped;
                OnDownloadFileCompleted(new DownloadAsyncCompletedEventArgs(TaskId, exp, true, Package));
            }
            catch (Exception exp)
            {
                Status = DownloadStatus.Failed;
                OnDownloadFileCompleted(new DownloadAsyncCompletedEventArgs(TaskId, exp, false, Package));
            }
            finally
            {
                _singleInstanceSemaphore.Release();
        ...
        private void SendDownloadCompletionSignal()
        {
            ...
            OnDownloadFileCompleted(new DownloadAsyncCompletedEventArgs(TaskId, null, false, Package));
        }
        ...
        {
            ChunkDownloader chunkDownloader = new ChunkDownloader(TaskId, chunk, Options, Package.Storage);
        ...
        private void OnDownloadFileCompleted(DownloadAsyncCompletedEventArgs e)
        {
            // flush streams
            Package.Flush();
        ...
        private void OnChunkDownloadProgressChanged(object sender, DownloadProgressChangedEventArgs e)
        {
            ...
            var totalProgressArg = new DownloadProgressChangedEventArgs(TaskId, nameof(DownloadService)) {
